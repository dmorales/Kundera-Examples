 ...
 */
/*
 * private static void findFriendTweets(Twissandra twissandra) { User user =
 * twissandra.findAUser("imp");
 * 
 * if (user != null) { // twissandra.findTweetsForFriends(user.getUserName(),
 * user.getId()); } else {
 * logger.warn("There is something wrong! User not found!!!"); }
 * twissandra.close(); } }
 */